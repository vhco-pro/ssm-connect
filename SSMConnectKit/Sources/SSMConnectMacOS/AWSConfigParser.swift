...
public struct AWSConfigParser {

    /// An `[sso-session NAME]` block.
    public struct SSOSession: Equatable {
        var startUrl: String?
        var region: String?
    }
    ...
    public func resolvedProfile(named profileName: String) -> ResolvedProfile? {
    ...
    public func resolvedProfiles() -> [(name: String, resolved: ResolvedProfile)] {
...
public extension AWSConfigParser {
    /// Convenience: load and parse `~/.aws/config` if it exists.
    public static func loadDefault(fileManager: FileManager = .default) -> AWSConfigParser? {
