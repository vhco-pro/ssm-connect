...
public protocol DCVViewerConfiguring: Sendable {
...
public struct GLibKeyfileDCVViewerConfigurator: DCVViewerConfiguring {
    public init() {}

    public static let group = "com/nicesoftware/DcvViewer"
    public static let key = "enable-high-pixel-density"

    /// GLib's keyfile-backend path. Overridable for tests.
    public var keyfileURL: URL = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".config/glib-2.0/settings/keyfile")

    public func ensurePreferredSettings(backingScale: CGFloat) {
    ...
    public static func enableHiDPIIfAbsent(in content: String) -> String? {
        let header = "[\(group)]"
