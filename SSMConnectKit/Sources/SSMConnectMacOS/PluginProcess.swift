...
public protocol SpawnedPluginProcess: AnyObject {
...
public protocol PluginSpawning: Sendable {
...
public final class ProcessPluginProcess: SpawnedPluginProcess, @unchecked Sendable {
    ...
    public func sendSIGTERM() { kill(process.processIdentifier, SIGTERM) }
    public func sendSIGKILL() { kill(process.processIdentifier, SIGKILL) }

    public func capturedStderr() -> String {
        lock.lock(); defer { lock.unlock() }
        return String(decoding: stderrBuffer, as: UTF8.self)
    }
}

public struct ProcessPluginSpawner: PluginSpawning {
    public init() {}

    public func spawn(executablePath: String, arguments: [String]) -> SpawnedPluginProcess {
...
public struct SystemPortProbe: PortProbing {
    public init() {}

    public func occupant(of port: Int) -> PortOccupant? {
