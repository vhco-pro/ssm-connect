...
public final class ConnectionStateMachine {
    ...
    public private(set) var errorCategory: ErrorCategory = .none
    ...
    public private(set) var instanceState: EC2Instance.State?
    public private(set) var tunnelPID: Int32?
    ...
    public private(set) var credentialsExpiry: Date?
    ...
    /// Build the flow from its ports.
    ///
    /// Every adapter is a required argument. Before the target split these had production defaults
    /// (`AWSAuthProvider()`, `DCVLauncher()`, …), which read as convenience but made the workflow
    /// depend on the AWS and macOS adapters — the exact dependency §5.2 forbids, and a cycle once
    /// those became separate targets. Construction now belongs to a composition root:
    /// `ConnectionStateMachine.production(profile:settings:)` in `SSMConnectKit` (MR-03).
    ///
    /// Only genuinely portable dependencies keep defaults, and each is inert rather than
    /// production behavior: a buffer-only log, a no-op interpreter, no notifications.
    public init(
        authProvider: AuthProviding,
        ec2: EC2Providing,
        ssm: SSMProviding,
        tunnel: TunnelProvider,
        secrets: SecretsProviding,
        identity: IdentityProviding,
        agent: AgentClienting,
        dcv: DCVLaunching,
        ...
        errorInterpreter: ErrorInterpreting = DefaultErrorInterpreter(),
    ...
    public func terminateTunnelForQuit() {
        monitorTask?.cancel()
    ...
    public var tunnelActive: Bool { currentHandle?.isActive ?? false }
    ...
    public func awaitInFlightTask() async {
    ...
    ) async throws -> EnsureSessionResult {
    ...
    public nonisolated static func defaultExpiredCredentialsCheck(_ error: Error) -> Bool {
