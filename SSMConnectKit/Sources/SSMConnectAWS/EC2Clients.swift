...
public protocol EC2Clienting: Sendable {
...
extension EC2Client: EC2Clienting {
    public func describeInstances(_ input: DescribeInstancesInput) async throws -> DescribeInstancesOutput {
        try await describeInstances(input: input)
    }
    public func startInstances(_ input: StartInstancesInput) async throws -> StartInstancesOutput {
        try await startInstances(input: input)
    }
    public func stopInstances(_ input: StopInstancesInput) async throws -> StopInstancesOutput {
        try await stopInstances(input: input)
    }
}

/// Builds a real `EC2Client` bound to explicit SSO STS credentials and the resource region.
public enum EC2ClientFactory {
    public static func make(credentials: AWSCredentials, region: String) throws -> EC2Clienting {
