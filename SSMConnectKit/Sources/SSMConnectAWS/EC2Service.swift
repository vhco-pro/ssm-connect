...
public final class EC2Service: EC2Providing {
    public typealias ClientFactory = @Sendable (_ credentials: AWSCredentials, _ region: String) throws -> EC2Clienting

    private let makeClient: ClientFactory

    public init(makeClient: @escaping ClientFactory = { try EC2ClientFactory.make(credentials: $0, region: $1) }) {
    ...
    public func startInstance(instanceId: String, region: String, credentials: AWSCredentials) async throws {
        let client = try makeClient(credentials, region)
        _ = try await client.startInstances(StartInstancesInput(instanceIds: [instanceId]))
    }

    public func stopInstance(instanceId: String, region: String, credentials: AWSCredentials) async throws {
        let client = try makeClient(credentials, region)
