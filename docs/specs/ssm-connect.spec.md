...
|-------------|------------------------------------------------------------|
| Version     | 1.2                                                        |
| Status      | Draft (design settled — ready for planner)                 |
...
| F-10 | P0 | **Launch DCV Viewer.** Once the tunnel is confirmed active (first successful health-check on `localhost:<localPort>`), the app launches Amazon DCV Viewer (`/Applications/DCV Viewer.app`) pointed at `https://localhost:<localPort>`. It uses `NSWorkspace.shared.open(_:configuration:)` or the `dcv://` URL scheme if registered by the DCV Viewer. | User: `ec2-user`. The app does NOT auto-fill the password into DCV Viewer (DCV Viewer has no automation API for credential injection). The password is shown in the menu for manual copy (F-11) |
| F-11 | P0 | **DCV password fetch and copy.** The app calls `SecretsManager.GetSecretValue` for secret ID `ec2/workstation-dcv-password` and holds the plaintext password in memory ONLY. It is displayed (masked, reveal-on-click) in the menu dropdown and a "Copy Password" button copies it to the macOS clipboard (`NSPasteboard`). The password is NEVER written to disk, UserDefaults, or Keychain. | Optional: auto-clear clipboard after 30 s (configurable; see NF-03). Fetch happens once per connect; re-fetched on manual reconnect |
...
| NF-14 | Observability | **Structured logging.** The app uses `os.Logger` (Apple Unified Logging) with subsystem `pro.vhco.ssm-connect` and categories `auth`, `ec2`, `ssm`, `tunnel`, `ui`. Logs are viewable in Console.app. Sensitive values (passwords, tokens) are logged as `<private>` using `OSLogPrivacy` |
...
| F-10 | **Given** the tunnel is active, **when** the app launches DCV Viewer, **then** `/Applications/DCV Viewer.app` opens pointed at `https://localhost:8443` |
| F-11 | **Given** the tunnel is active, **when** the user clicks "Copy Password" in the menu, **then** the DCV password from Secrets Manager is copied to the clipboard. The password is not present in any file on disk |
...
| **DCV Viewer not installed** | Alert: "Amazon DCV Viewer is required but not found at /Applications/DCV Viewer.app. Install via `brew install --cask dcv-viewer` or download from https://download.amazondcv.com." The tunnel remains active — the user can install DCV Viewer and manually connect |
| **DCV Viewer already running** | `NSWorkspace.open` with the URL will either open a new connection tab or bring the existing one to front. No special handling needed |
...
| A | Homebrew tap repository | **`vhco-pro/homebrew-tap`** (private), authored under `michielvha`. A private tap works with the user's existing `gh`/git credentials. Decide cask token at first release |
| B | App identity | **Name "SSM Connect", bundle id `pro.vhco.ssm-connect`, repo [`vhco-pro/ssm-connect`](https://github.com/vhco-pro/ssm-connect)** (private). Menu-bar icon set TBD during implementation (placeholder SF Symbol `bolt.horizontal.circle`) |
