...
- `gitversion.yml` (GitHubFlow, **`v` tag prefix**, `increment: None` + ContinuousDelivery) derives the next semantic version from conventional-commit messages (`feat:` → minor, `fix:`/`perf:`/`refactor:` → patch, `BREAKING CHANGE`/`!:` → major, `chore`/`docs`/`ci`/etc → **no bump → same version → no release**).
