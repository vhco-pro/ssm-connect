...
- **Status:** draft (for review) — spec step of the SDD loop
...
| RVL-3 | P0 | **Tunnel-up assertion before probing.** Before the readiness probe, assert the port-forward is actually listening on `127.0.0.1:<localPort>` (IPv4). A half-open / failed `session-manager-plugin` MUST surface as a distinct `tunnelNotEstablished` error, not as a viewer error. | The plugin binds IPv4 `127.0.0.1`; assert against that exact endpoint (ties to #11). |
