---
status: complete
status_description: "Phases A–H COMPLETE — build + 101 tests green. A: scaffold+plugin. B: SSO auth (cache/refresh/device-auth, refresh-failure→browser fallback). C: EC2 service (resolve-by-tag, start/stop, poll-until-running) +8 tests. D: SSM service (online-poll + StartSession port-forward) + BundledPluginTunnel (5-arg plugin exec, port-in-use guard, plugin validation, SIGTERM→SIGKILL lifecycle, onDisconnect) +13 tests. E: Secrets fetch + DCV auto-login via transient 0600 .dcv connection file (ADR-8) + clipboard copy/auto-clear +15 tests. F: ConnectionStateMachine orchestrates all 8 states end-to-end (auto-start F-07, per-stage timeouts F-06, tunnel-drop auto-reconnect 3×/5s F-13, SSO-expiry re-auth without tunnel teardown F-17, stop-workstation F-15, best-effort DCV F-16) +16 tests. G: ProfileStore (UserDefaults, no secrets) + AWSConfigParser (sso-session+legacy formats) + first-launch seeder + SettingsView/ProfileEditorView (multi-profile CRUD, login-item toggle via SMAppService) +16 tests. H: structured os.Logger (subsystem pro.vhco.ssm-connect, 5 categories) + 200-line RingBuffer connection log + LogView window + UNUserNotification events (connected/stopped/reconnecting/sign-in) + enriched menu (instance/state/tunnel pid+port/elapsed/masked password) + Stop-Workstation confirmation +12 tests. Menu wired to state machine + Settings scene. Dev tooling: scripts/run.sh + Makefile. C6/B8 live + D10/E/F8/G10 live-manual pending. Review (2026-06-06) fixed 5 HIGH bugs: plugin stdout now drained (tunnel-stall), connect-task re-entrancy via generation token, auto-reconnect tears down before Error (no orphan plugin), device-auth poll no longer busy-spins, clipboard auto-clear setting now wired (0=disabled) — build + 101 tests still green. Phase I in progress: I1 done (Phase A); I2/I3 done — entitlements file (network.client) + Release-gated re-sign that re-seals plugin+bundle after embed (fixes broken-seal hazard). I4 (archive/export), I5/I6 (Homebrew cask + tap repo), I7 (notarization docs), I8 (brew e2e) remaining. Live E2E (2026-06-06): full flow validated against workstation-prd workstation — SSO → resolve i-01a64… (stopped) → start → SSM online → tunnel localhost:8443 → connected, all green (C6/D10/F8 effectively confirmed). DCV launch initially failed with `dns resolution (domain: 'file')`; root-caused to the immediate `.dcv` deletion racing DCV's async read, fixed via grace-period delete (E5/ADR-8). Further fixes (2026-06-06): (1) SSO browser-every-connect — device-auth tokens are now persisted to `~/.aws/sso/cache` (CLI-compatible) so connects reuse/refresh silently; (2) DCV `endpoint unreachable` — added an HTTPS readiness probe so the viewer launches only once the in-VM DCV server answers; (3) **generalized config (brew pre-req)** — stripped all hardcoded AWS env from the binary; first launch imports a profile from `~/.aws/config` (user picks; tag/secret filled by user). De-branded the binary (os.Logger subsystem com.oneb2c.workstation → pro.vhco.ssm-connect; generic placeholders). Phase I shipped: `brew install --cask vhco-pro/tap/ssm-connect` verified end-to-end. Live-test follow-up: plugin lifecycle bug fixed (F-13/§8) — Quit now tears down the plugin via AppDelegate.applicationWillTerminate (was orphaning it holding port 8443), and the port-in-use guard reclaims a stale OWN plugin instead of erroring. Open-sourced under Apache-2.0 (binary/docs/git-history scrubbed of AWS identifiers). Phase J: GitVersion CI/CD — a single self-contained macOS `release.yml` (no GitHub App, just GITHUB_TOKEN) computes the version on every push to main and, if bump-worthy, builds + publishes the GitHub Release + (optionally, via TAP_TOKEN) bumps the cask. The pipeline owns versioning/release; manual v0.1.0 removed. 104 tests green."
...
- [x] Task E5 — Implement `DCVLauncher.launch`: write the `.dcv` to a `0600` temp file, open it with DCV Viewer, then **delete after a short grace period** so the viewer reads it first (immediate delete on launch failure); injectable locator/store/opener seams + injectable `cleanupDelay` (F-10, ADR-8). _(Grace delay added 2026-06-06: immediate deletion raced DCV's async read and surfaced a bogus `dns resolution (domain: 'file')` error — confirmed by reproduction; persistent file connects + reaches login. See ADR-8 implementation note.)_
...
**Tasks**:
- [x] Task G1 — Implement `ConnectionProfile` model: `Codable` struct with all fields per F-18 (display name, SSO start URL, SSO region, account ID, role name, resource region, tag key, tag value, secret ID, local port, remote port default 8443, connect action)
- [x] Task G2 — Implement `AppSettings` model: auto-connect, auto-reconnect, clipboard auto-clear delay (F-18)
- [x] Task G3 — Implement `ProfileStore`: read/write list of profiles + settings to `UserDefaults` (no secrets — NF-01, F-18)
- [x] Task G4 — Implement `AWSConfigParser`: parse `~/.aws/config` for `[sso-session NAME]` blocks (sso_start_url, sso_region, sso_account_id, sso_role_name) and `[profile NAME]` blocks with `sso_session=NAME` references. Support the newer sso-session format confirmed in the spike (spec §12.1)
- [x] Task G5 — First-launch config import (generalized 2026-06-06 — **no AWS env baked into the binary**, a brew-distribution pre-req): the app ships with **zero** profiles and `ConnectionProfile.template` is blank. On first launch the menu/Settings guide the user to **import from `~/.aws/config`** — `ProfileStore.importableProfiles()` lists every `[profile]`/`[sso-session]` (SSO start URL, SSO region, account, role, resource region pre-filled); the user picks one and fills the instance tag value + DCV secret id (config doesn't carry those). Auto-connect is gated on `profile.isConfigured`. Concrete example values now live only in the test target (`ConnectionProfile.example`). (F-18, ADR-5)
- [x] Task G6 — Implement `SettingsView`: SwiftUI Settings scene with profile list, add/edit/duplicate/delete, active-profile selector, global settings toggles. Collapse to single-profile view when only one profile exists (F-18, ADR-5)
- [x] Task G7 — Implement `ProfileEditorView`: form for all profile fields with validation (port range, non-empty required fields)
- [x] Task G8 — Implement login-item toggle: `SMAppService.mainApp.register()` / `.unregister()`, query `.status` to sync UI state with System Settings (F-02, ADR-3)
- [x] Task G9 — Unit tests: `AWSConfigParser` with fixture config files (sso-session format, legacy format, missing blocks); `ConnectionProfile` Codable round-trip; `ProfileStore` CRUD; login-item status query
...
**Tasks**:
- [x] Task I1 — Finalize `scripts/fetch-plugin.sh`: pin `session-manager-plugin` version `1.2.814.0`, hardcode the expected SHA-256 checksum, fail build on mismatch (ADR-7). _(Done in Phase A: version + pkg/bin SHA-256 pinned, hard-fail on mismatch.)_
- [x] Task I2 — Code sign the plugin binary: `codesign --force --sign -` on `Contents/Helpers/session-manager-plugin` (re-sign ad-hoc to match the app's signing context) (NF-06). _(Done in the `Embed & re-sign` post-build phase, Release-gated.)_
- [x] Task I3 — Code sign the app bundle: `codesign --force --deep --sign - --entitlements …` as the LAST step after the plugin is embedded (fixes the broken-seal-from-embed hazard). Added `SSMConnect/SSMConnect.entitlements` with `com.apple.security.network.client`, wired via `CODE_SIGN_ENTITLEMENTS` (NF-04, NF-06). Verified: Debug build applies the entitlement + valid signature; Release re-sign seals plugin + bundle.
- [x] Task I4 — `scripts/release.sh`: clean Release build → `dist/SSMConnect-<version>.zip` (ditto, preserves signature) + SHA-256. Published as a GitHub release asset on `vhco-pro/ssm-connect` (v0.1.0, sha `bb82f035…40c8c3`).
- [x] Task I5 — Homebrew cask `vhco-pro/homebrew-tap` → `Casks/ssm-connect.rb` (version 0.1.0, sha256, release-asset URL, `app "SSMConnect.app"`, Gatekeeper caveats, `zap` of the prefs plist). `brew fetch` verifies the download + sha; `brew install --cask vhco-pro/tap/ssm-connect` succeeds.
- [x] Task I6 — Gatekeeper one-time approval documented in the cask caveats + README (`xattr -dr com.apple.quarantine` or right-click → Open) (NF-06).
- [x] Task I7 — Notarization upgrade path documented in the README (Developer ID + `notarytool submit` + `stapler staple`) (NF-04, NF-06, ADR-4).
- [x] Task I8 — End-to-end verified: `brew install --cask vhco-pro/tap/ssm-connect` → app in `/Applications`, signature valid (`codesign --verify --deep --strict`), launches from the menu bar after Gatekeeper approval. (Live full connection flow already validated against the workstation pre-distribution.)

> **Open-sourced 2026-06-06:** `vhco-pro/ssm-connect` made public under Apache-2.0. All AWS-environment identifiers (account id, SSO URL, instance tag, org name) were stripped from the binary, docs, **and git history** (filter-repo + force-push) — none were credentials; the user's own profile lives only in local UserDefaults.
...
### Phase J: CI/CD — Automated Versioning & Release (ADR-9)

**Priority: MEDIUM** — Removes the manual release toil; the pipeline owns versioning + distribution.

**Goal**: Merging conventional commits to `main` cuts a `vX.Y.Z` GitHub Release and updates the Homebrew cask automatically — no manual version picking, tagging, artifact upload, or cask edit.
...
- [x] Task J2 — `.github/workflows/release.yml` — **single job, no GitHub App** (just `GITHUB_TOKEN` — GitVersion only needs a token with `contents:write` to push the tag; the App is only for chaining a separate tag-triggered workflow, which a single job avoids). On every push to `main`: compute the version with GitVersion and gate on whether `vX.Y.Z` is tagged; only on a new version, tag it with the reusable **`michielvha/gitversion-tag-action`** (`prefix: v`), stamp `Info.plist`, build via `scripts/release.sh`, and `gh release create` with the zip asset. No-bump pushes are skipped.
...
**Verifiable by**: a push to `main` with a `feat:`/`fix:` commit produces a new `vX.Y.Z` GitHub Release; `brew upgrade` pulls it (cask bumped automatically with `TAP_TOKEN`, else one manual line).
