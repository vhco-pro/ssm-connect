...
- [x] Task J2 — `.github/workflows/tag.yml`: on push to `main`, mint a `RELEASE_BOT` GitHub App token and run `michielvha/gitversion-tag-action` (`prefix: v`) to compute + push the version tag (App token, so the tag triggers the release).
- [x] Task J3 — `.github/workflows/release.yml`: on a `v*.*.*` tag, on `macos-14` — stamp `Info.plist` from the tag, build the ad-hoc-signed `.app` via `scripts/release.sh`, publish the GitHub Release with the zip asset, and bump the cask (`version`+`sha256`) in `vhco-pro/homebrew-tap`.
...
- [ ] Task J5 — Prereqs (author-managed): install the `RELEASE_BOT` GitHub App on `vhco-pro/ssm-connect` **and** `vhco-pro/homebrew-tap`; ensure `RELEASE_BOT_APP_ID` / `RELEASE_BOT_PRIVATE_KEY` secrets are available to the repo.

**Depends on**: Phase I (release.sh, cask, tap).

**Verifiable by**: a merge to `main` produces a new `vX.Y.Z` release + the cask updates; `brew upgrade` pulls it.
